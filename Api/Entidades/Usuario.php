        ...
        */
        ...
        static function ListarLogsUsuario($userId=0,$from=null,$to=null){
            ...
            if($from !=null && $to != null)
            {
                ...
                {
                    if($userId == 0)
                        $consulta = $objDB->RetornarConsulta("SELECT usuarios.id,nombre,apellido,estado,admin,turno,log.entrada FROM usuarios, logusuarios AS log WHERE log.idUsuario=usuarios.id AND cast(log.entrada as date) BETWEEN   '".$from."'  AND  '".$to."'");
                    else
                    {
                        $consulta = $objDB->RetornarConsulta("SELECT usuarios.id,nombre,apellido,estado,admin,turno,log.entrada FROM usuarios, logusuarios AS log WHERE log.idUsuario=usuarios.id AND usuarios.id=:Id AND cast(log.entrada as date) BETWEEN   '".$from."'  AND  '".$to."' ");
                ...
                {
                    if($userId == 0)
                        $consulta = $objDB->RetornarConsulta("SELECT usuarios.id,nombre,apellido,estado,admin,turno,log.entrada FROM usuarios, logusuarios AS log WHERE log.idUsuario=usuarios.id AND cast(log.entrada as datetime) BETWEEN   '".$from."'  AND  '".$to."'");
                    else
                    {
                        $consulta = $objDB->RetornarConsulta("SELECT usuarios.id,nombre,apellido,estado,admin,turno,log.entrada FROM usuarios, logusuarios AS log WHERE log.idUsuario=usuarios.id AND usuarios.id=:Id AND cast(log.entrada as datetime) BETWEEN   '".$from."'  AND  '".$to."' ");
                        $consulta->bindValue(':Id',$userId, PDO::PARAM_INT);
                    }
                }
            ...
            elseif($from !=null)
            {
                if($userId == 0)
                {
                    $consulta = $objDB->RetornarConsulta("SELECT usuarios.id,nombre,apellido,estado,admin,turno,log.entrada FROM usuarios, logusuarios AS log WHERE log.idUsuario=usuarios.id AND log.entrada LIKE '%".$from."%'");
                    $consulta->bindValue(':dFrom',$from, PDO::PARAM_STR);
                }
                else
                {
                    $consulta = $objDB->RetornarConsulta("SELECT usuarios.id,nombre,apellido,estado,admin,turno,log.entrada FROM usuarios, logusuarios AS log WHERE log.idUsuario=usuarios.id AND usuarios.id=:Id AND log.entrada LIKE '%".$from."%'");
                    $consulta->bindValue(':Id',$userId, PDO::PARAM_INT);
                    $consulta->bindValue(':dFrom',$from, PDO::PARAM_STR);
                }
            }
            elseif($userId!=0)
            {
                $consulta = $objDB->RetornarConsulta("SELECT usuarios.id,nombre,apellido,estado,admin,turno,log.entrada FROM usuarios, logusuarios AS log WHERE log.idUsuario=usuarios.id AND usuarios.id=:Id");
                $consulta->bindValue(':Id',$userId, PDO::PARAM_INT);

            }
            else
                $consulta = $objDB->RetornarConsulta("SELECT usuarios.id,nombre,apellido,estado,admin,turno,log.entrada FROM usuarios, logusuarios AS log WHERE log.idUsuario=usuarios.id  ORDER BY usuarios.id");
        $consulta->execute();
